module.exports = async (req, res) => {
  if (!req.query) return res.status(400).send("Please set the query params!");
  try {
    const params = [decodeURIComponent(req.query.title), decodeURIComponent(req.query.image), decodeURIComponent(req.query.driver), decodeURIComponent(req.query.passenger), decodeURIComponent(req.query.rear)]
    const key = ["{{title}}", "{{image}}", "{{driver}}", "{{passenger}}", "{{rear}}"]
